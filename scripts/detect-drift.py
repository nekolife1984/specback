...
def load_source_map(path: Path) -> dict[str, Any]:
    ...
    Missing file → ERROR + exit(1) (unchanged).  Index construction lives
    in :func:`artifact_io.load_source_map` (Issue #283).
    """
    if not path.exists():
        print(
...
def load_trace(path: Path) -> dict[str, Any]:
    """Load trace.json (missing file → ERROR + exit(1), unchanged).

    Delegates to :func:`artifact_io.load_trace` (Issue #283).
    """
...
def compute_hash_changes(
    ...
    for sid, stored in units.items():
        ...
        try:
            ...
            current_digest, current_line_count = "", 0
...
def generate_markdown(
    ...
    lines: list[str] = []
    ts = utcnow_iso_z()
    san = sanitize_control  # repo-controlled strings → escape C0 control chars
    ...
    if affected:
        ...
        for spec_file in sorted(by_spec_file):
            ...
            for section_name in sorted(by_section):
                ...
                lines.append(f"#### {icon} {san(section_name)}")
                lines.append("")
                for src in sources:
                    src_ids_str = ", ".join(san(sid) for sid in src["src_ids"][:5])
                    more = f" and {len(src['src_ids'])-5} more" if len(src['src_ids']) > 5 else ""
                    lines.append(
                        f"- **{san(src['source_file'])}** ({san(src['status'])}) "
                        f"— {san(src['impact'])} impact"
    ...
    if deleted:
        ...
        lines.append("")
        for entry in deleted:
            lines.append(f"### `{san(entry['file'])}`")
            lines.append("")
            for sec in entry.get("impacted_sections", []):
                lines.append(
                    f"- Referenced in `{san(sec['file'])}` §{san(sec['section'])} — "
    ...
    if new_uncovered:
        ...
        for entry in new_uncovered:
            lines.append(
                f"| `{san(entry['file'])}` | {san(CHANGE_TYPES.get(entry['status'], entry['status']))} "
                f"| {san(entry.get('reason', ''))} |"
            )
        lines.append("")
        lines.append("---")
        lines.append("")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not changes:
        ...
        atomic_write_text(output_path, empty_md)
        ...
        if args.json:
            ...
            atomic_write_json(json_path, empty_json)
    ...
    output_path = args.output or str(output_dir / "drift-report.md")
    atomic_write_text(output_path, md)
