...
- インクリメンタル仕様更新 ([#269]): `scripts/specback-incremental-update.py`（Phase 7 の `drift-report.json` から影響を受けた章だけを特定し、章単位の再調査プロンプトを生成する `plan`、SRC-ID 再採番トラップガード + 巻き込み変更ゼロ検証を行う `verify`、バックアップ + アトミック置換 + `build-trace.py` 再実行を行う `apply`。`--json` / `--skip-trace-refresh` 対応、state.json ベースラインで冪等）。Phase 7e として `skills/specback/phases/phase-7e-incremental-update.md` に組み込み、docs EN/JA 追加
- 仕様ヘルスレポート ([#268]): `scripts/specback-health.py`（章ファイルの確度ラベル 🟢/🟡/🔴・REF・未解決項目と、`questions.json` / `trace.json` / `drift-report.json` / `state.json` / `coverage-check.py --output-format json` を集約し、章別・全体の「仕様ヘルススコア」を1枚の Markdown スコアカードとして出力。`--json` / `--min-health-score` / `--assumed-ratio-threshold` に対応し、🔴 密度が閾値を超える章は Phase 5 精緻化へ誘導。欠損メトリクスは重みを再正規化して継続）。Phase 6 納品前の必須チェック（`phase-6-deliver.md` ステップ 6）に組み込み、docs EN/JA 追加
- specback-search MCP サーバー ([#270]): `skills/specback-search/scripts/specback-search-mcp.py`（`build-search-index.py` をラップする MCP stdio サーバー。stdlib only・外部依存なし・改行区切り JSON-RPC 2.0 で、`specback_search` / `specback_uncovered` / `specback_drift` / `specback_questions` の4ツールを Claude Code / Cursor / Hermes 等のエージェントに公開。`specback_dir` をツール引数で受け取り、`text`/`json` 両形式に対応。ツール失敗時もサーバーは生存）。トランスポートテスト含むテスト一式、CI・testpaths に specback-search テストを追加、docs EN/JA 追加
- テンプレートの機械可読カタログ ([#299]): `templates/catalog.json`（9テンプレートの `name` / `description` / `template_version` / `chapters` / `detection_rules` / `languages` を登録。`### Chapter N: Title` 見出しと frontmatter の `detection_rules` から同期）。`scripts/validate-template-catalog.py`（catalog と `templates/*.md` の整合検証: ファイル存在 / frontmatter 同一性 / 章構成（順序含む） / detection_rules のIDリスト / deprecated `[REF: ...]` 拒否 / 言語の既知セット検証。exit 0/1/2）。phase-1-recon のテンプレート候補提示と detection rules 読み込みで catalog 参照を追加、docs EN/JA 追加
- テンプレートカタログ検証の堅牢化 ([#299] follow-up, PR #300 事後レビュー対応): `validate-template-catalog.py` に ReDoS 修正（章見出し regex の greedy 化。100KB行で36秒→即時）、非UTF-8/深ネストJSON/NaN のクリーンエラー化（`reject_nonfinite` 流用）、テンプレート名ホワイトリスト+パストラバーサル/symlink 拒否、制御文字サニタイズ（`sanitize_control`）、quoted scalar 対応、欠落フィールドの warning 化、detection_rules 抽出の状態汚染防止。テスト17件追加（計48件）。`.githooks/pre-commit` Phase 4 と CI にカタログ検証を配線（スクリプト変更なしでも docs/templates のみ変更時に検証が走るよう早期 exit を除去）
- 機能仕様2.2章の受入シナリオ(BDD)・優先度・独立テスト ([#298]): 全9テンプレートの per-feature 定義に `Priority`（P1/P2/P3）/ `Acceptance scenarios`（Given/When/Then + REF）/ `Independent test` / `Edge cases` の4セクションを追加（event-driven は 2.3 として新設）。エージェント指示（`agents/chapter-investigator.md` STEP J / `variants/B/` STEP C / `phase-3a-comprehensive.md` STEP C）に追記し、docs EN/JA 同期
