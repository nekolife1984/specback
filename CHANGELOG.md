...
- 共有マイクロヘルパー基盤 ([#279]): `scripts/common.py`（`utcnow_iso` / `load_json_text` / `atomic_write_json` / `sha256_file` / `add_specback_dir_arg`）。scripts/ 全体で別実装ごとにバラバラだった時刻（~22箇所）・JSON読み込み（~7実装）・SHA-256（~6実装）・`--specback-dir` ボイラープレート（~16箇所）を集約する基盤モジュール。堅牢化済み実装（バウンド読み・NaN/Infinity拒否・symlink拒否・アトミック書き込み、#267/#273 準拠）を踏襲し、`load_json_text` は sys.exit せず raise する（共有モジュールは import-safe 必須、#270）。**基盤の新設のみ**で、各スクリプトへの配線は後続 Issue で実施
...
[#279]: https://github.com/nekolife1984/specback/issues/279
