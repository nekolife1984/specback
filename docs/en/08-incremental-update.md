...
| Transactional rollback | chapter + trace are backed up together; a trace-refresh failure restores both to their pre-apply state (`rollback` in `--json` result) |
| Baseline refresh | a successful apply re-snapshots the chapter's hash in `state.json`, so N chapters from one `plan` apply sequentially without the earlier chapters becoming false collateral edits |
